...
async def cmd_start(message: types.Message):
    ...
    keyboard.add(types.InlineKeyboardButton("⭐️ Поддержите нас", callback_data="donate"))
...
async def start_search(callback_query: types.CallbackQuery):
    search_texts = {
        "search_anime": "Загружаем страницу аниме... ⏳⌛️⏳",
        "search_movies": "Загружаем страницу фильмов... ⏳⌛️⏳",
        "search_series": "Загружаем страницу сериалов... ⏳⌛️⏳",
        "search_cartoons": "Загружаем страницу мультфильмов... ⏳⌛️⏳"
    }
    search_functions = {
...
async def donate_stars(callback_query: types.CallbackQuery):
    donation_keyboard = types.InlineKeyboardMarkup(row_width=2)
    donation_keyboard.add(
    ...
    donation_keyboard.add(types.InlineKeyboardButton("🏠 Главное меню", callback_data="main_menu"))
    try:
        await bot.edit_message_text(
            ...
            text=(
                ...
                "⚠️ Перед тем как вы решитесь поддержать наш проект, прочитайте наши условия возврата средств - /paysupport\n\n"
                "<b>Пожалуйста выберите сумму для доната:</b>"
            ),
            parse_mode=types.ParseMode.HTML,
